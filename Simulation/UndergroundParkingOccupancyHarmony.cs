...
namespace UndergroundParkingGarage
{
    internal static class UndergroundParkingOccupancyHarmony
    {
        ...
        private static readonly ushort[] ManagedDepartureParkedIds = new ushort[65536];
        ...
        private static readonly object ManagedDepartureAdmissionSync = new object();
        private static readonly Dictionary<int, uint> ManagedDepartureAdmissions =
        ...
        private static int _managedRetrievalScopeDepth;
        private static ushort _managedRetrievalScopeParkedId;
        ...
        public static void Release()
        {
            ...
            Array.Clear(ManagedDepartureParkedIds, 0, ManagedDepartureParkedIds.Length);
            ...
            lock (ManagedDepartureAdmissionSync)
                ManagedDepartureAdmissions.Clear();
            ...
            _managedRetrievalScopeDepth = 0;
            _managedRetrievalScopeParkedId = 0;
            ...
            ref bool __result)
        {
            ...
            if (UndergroundParkingEntryRouteManager.TryProbeRoadPortalArrival(
                ...
                && IsArrivalAdmissionOwned(pendingFacilityId))
            {
            ...
            out RoutedArrivalState state)
        {
            ...
            // PassengerCarAI.SimulationStep is a city-wide hot path. Reject
            // ordinary cars before interpolating their render pose or querying
            // any TM:PE/facility state; only a vehicle with an adopted UPG route
            // can possibly be at the validated entrance transaction boundary.
            if (!UndergroundParkingEntryRouteManager.HasActiveRoute(vehicleID))
                return false;
            ...
            ref Vehicle data)
        {
            ...
            if (!UndergroundParkingVisualManager.IsFacilityVisibleOnCamera(
                    state.FacilityId))
            {
                // Camera visibility owns presentation only. TM:PE-controlled
                // arrivals have already retained the exact active vehicle and
                // followed their accepted native route to this validated stop.
                // Off-camera or high-camera arrivals now perform the identical
                // reserved-space transaction without allocating a proxy or
                // consuming the portal-animation FIFO.
                RoutedArrivalVisualsSuppressed[vehicleID] = true;
                RoutedArrivalAnimationStates[vehicleID] = ArrivalStateCommitRequested;
            ...
            RoutedArrivalState state)
        {
            ...
            else
            {
                ...
                if (!SuppressArrivalParkedVehicleAssociations(
                        vehicleID,
                        occupantCount))
                {
                    CancelPendingRoutedArrival(vehicleID, true);
        ...
        private static bool SuppressArrivalParkedVehicleAssociations(
            ushort arrivalVehicleId,
            int occupantCount)
        {
            CitizenManager citizenManager = CitizenManager.instance;
            if (citizenManager == null)
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                UndergroundParkingLifecycleDiagnostics.TraceCitizen(
                    ...
                    + " heldUntilOnwardPathEstablished=True"
            ...
            out string reason)
        {
            ...
            if (citizen.m_parkedVehicle != 0)
            {
                reason = "association-already-replaced";
                return true;
            }

            ushort instanceId = citizen.m_instance;
            if (instanceId != association.InstanceId)
            {
                // The quarantine belongs only to the exact pedestrian
                // continuation that left this car at the garage. Once that
                // instance ends or is replaced, a later trip must see the
                // citizen's normal parked-car ownership immediately. Following
                // the replacement instance until it becomes idle can restore
                // the garage car at an unrelated destination and trigger a
                // distant retrieval instead of TM:PE's ordinary parking retry.
                reason = "instance-replaced";
                return true;
            }

            if (citizen.m_vehicle != 0)
            {
                // The original arrival car can still own the citizen for the
                // few instructions between quarantine and pavement handoff.
                // Any different active vehicle belongs to a later lifecycle;
                // preserving the old hidden garage link through that journey
                // makes its terminal parking decision run without the normal
                // parked-car ownership.
                if (citizen.m_vehicle != association.ArrivalVehicleId)
                {
                    reason = "different-active-vehicle";
                    return true;
                }
                return false;
            }
            ...
            bool continuationActive = citizenInstance.m_path != 0u
                                      || (citizenInstance.m_flags
                                          & CitizenInstance.Flags.WaitingPath) != 0;
            if (continuationActive)
            {
                // A real or waiting native/TM:PE pedestrian path is the exact
                // transaction boundary: route selection has completed and
                // normal parked-car ownership can no longer redirect this
                // already-selected continuation. Restore now, while that path
                // is still identifiable, rather than polling for a later idle
                // state that can belong to a reused instance at an unrelated
                // destination.
                reason = "onward-path-established";
                return true;
            }

            // The cooldown remains only for a continuation that never becomes
            // observable. Replacement instances and different active vehicles
            // are handled above, so the fallback cannot intentionally adopt a
            // later journey as its completion signal.
            bool cooldownReached = HasReachedSimulationFrame(
                GetCurrentSimulationFrame(),
                association.EligibleFrame);
            if (cooldownReached)
                reason = "no-path-cooldown";
            ...
            string reason)
        {
            ...
            {
                UndergroundParkingLog.Advanced(
                    "UPG restored parked ownership after arrival handoff: citizen="
                    + citizenId
            ...
            ref Vehicle data)
        {
            ...
            if (IsAdoptedRoutedArrivalReleaseProtected(vehicleID))
            {
                ...
                if (UndergroundParkingEntryRouteManager.TryProbeRoadPortalArrival(
                        ...
                        out facilityId))
                {
                    if (!IsArrivalAdmissionOwned(facilityId))
                    {
            ...
            RoutedArrivalState state)
        {
            ...
            if (IsArrivalAdmissionOwned(state.FacilityId))
            {
        ...
        public static void MarkRoutedArrivalAnimationReady(ushort vehicleId)
        {
            ...
            bool controlledTmpeTraversal =
                UndergroundParkingEntryRouteManager.IsTmpeAdoptedArrival(
                    vehicleId);
            // Capture the pose rendered in this Unity update for both parking
            // modes. The ordinary commit retires the native record instead of
            // retaining it, but its replacement must begin from the identical
            // final body pose just as the TM:PE-controlled replacement does.
            ...
            {
                // Capture the pose used by this Unity update and request the
                // retained native body's unspawn immediately. Waiting for a
                // second render update made an otherwise free entrance look
                // like a compulsory stop. The proxy remains hidden until the
                // simulation thread acknowledges the unspawn, so there is
                // still exactly one visible and collidable movement owner.
                RequestRoutedArrivalNativeUnspawn(vehicleId);
                return;
            }
        ...
        public static void ObserveRoutedArrivalRenderPose(ushort vehicleId)
        {
            ...
            // exactly. This is the pose which the current Unity update would
            // render, so it is also the correct no-dwell proxy start. The
            // animation driver repeats this observation while it waits for the
            // simulation-thread unspawn acknowledgement. Once Spawned clears,
            // this method stops updating the stored pose, leaving the final
            // live native pose as the atomic handoff point.
            ...
            RoutedArrivalHandoffPositions[vehicleId] = renderedPosition;
            RoutedArrivalHandoffRotations[vehicleId] = renderedRotation;
            ...
            InstanceID ___m_hoverInstance)
        {
            ...
            bool hasTraffic = UndergroundParkingRegistry.HasFacilityActivity(facility.Id);
            if (parked <= 0 && !hasTraffic)
                return;
            ...
            string message = parked > 0
                ? "Cannot demolish: "
                  ...
                  + (parked == 1 ? "it" : "them")
                  + " to leave first."
                : "Cannot demolish yet: incoming or departing garage traffic is still using this entrance. Switch the car park off and wait for that traffic to finish.";
            ...
            if (_blockedDemolitionLogCount < 8)
            {
                ...
                UndergroundParkingLog.Advanced("UPG blocked demolition of protected entrance: facility="
                                            + facility.Id
                                            + " building="
                                            + building
                                            + " parked="
                                            + parked
                                            + " activity="
                                            + UndergroundParkingRegistry.DescribeFacilityActivity(facility.Id));
            }
        }
            ...
            out ManagedRetrievalState __state)
        {
            ...
            if (HandleNestedManagedRetrieval(parkedId, out __state))
                return;
            ...
            __state = BeginManagedRetrievalScope(
                parkedId,
                originalPosition,
                originalRotation);
            UndergroundParkingLifecycleDiagnostics.TraceCitizen(
        ...
        {
            CompleteManagedRetrievalScope(__state, false);
            return __exception;
        }
        ...
        private static bool TmpeEnterParkedCarPrefix(
            ushort instanceId,
        ...
        {
            __state = default(ManagedRetrievalState);
            if (HandleNestedManagedRetrieval(parkedVehicleId, out __state))
                return true;
            ...
            _managedDepartureContext = new ManagedDepartureContext(
                ...
                facility,
                connection);
            if (!TryReserveManagedDepartureAdmission(
                    parkedVehicleId,
                    facility,
                    connection))
            {
        ...
        private static void OwnerSpawnVehiclePostfix(
            bool __result,
            ManagedRetrievalState __state)
        {
            if (!__state.IsManaged)
                return;
            ...
            ManagedDepartureParkedIds[vehicleId] = departure.ParkedId;
            TransferManagedDepartureAdmission(
                departure.Facility.Id,
                departure.ParkedId,
                vehicleId);
            AlignManagedDepartureVehicleForPublication(
                ...
                releasePosition,
                ManagedDepartureReleaseRotations[vehicleId]);
            RequestManagedDepartureNativePath(vehicleId, ref vehicleData);
        ...
        private static Exception OwnerSpawnVehicleFinalizer(
            ...
            ManagedRetrievalState __state)
        {
            CompleteManagedRetrievalScope(__state, true);
            return __exception;
        }

        internal static bool HasManagedDepartureAdmission(int facilityId)
        {
            if (facilityId <= 0)
                return false;
            lock (ManagedDepartureAdmissionSync)
                return ManagedDepartureAdmissions.ContainsKey(facilityId);
        }

        private static bool IsArrivalAdmissionOwned(int facilityId)
        {
            return facilityId > 0
                   && UndergroundParkingPortalAnimationManager
                       .HasArrivalActivityForFacility(facilityId);
        }

        private static bool TryReserveManagedDepartureAdmission(
            ushort parkedId,
            UndergroundParkingFacility facility,
            UndergroundParkingRoadConnection entranceConnection)
        {
            if (parkedId == 0
                ...
                || UndergroundParkingPortalAnimationManager
                    .HasDepartureActivityForFacility(facility.Id)
                || !HasManagedDepartureAdmissionLaneSpace(
                    parkedId,
                    facility,
                    entranceConnection))
            {
            ...
            lock (ManagedDepartureAdmissionSync)
            {
                if (ManagedDepartureAdmissions.ContainsKey(facility.Id))
                    return false;
                ManagedDepartureAdmissions[facility.Id] = parkedId;
                return true;
            }
        }

        private static bool HasManagedDepartureAdmissionLaneSpace(
            ushort parkedId,
            UndergroundParkingFacility facility,
            UndergroundParkingRoadConnection entranceConnection)
        {
            ...
            float vehicleLength = info.m_generatedInfo == null
                ? 8f
        ...
        private static void TransferManagedDepartureAdmission(
            int facilityId,
            ushort parkedId,
            ushort vehicleId)
        {
            if (facilityId <= 0 || parkedId == 0 || vehicleId == 0)
                return;
            lock (ManagedDepartureAdmissionSync)
            {
                uint token;
                if (ManagedDepartureAdmissions.TryGetValue(facilityId, out token)
                    && token == parkedId)
                {
                    ManagedDepartureAdmissions[facilityId] = 65536u + vehicleId;
                }
            }
        }

        private static void ReleaseManagedDepartureAdmission(ushort parkedId)
        {
            if (parkedId == 0)
                return;
            lock (ManagedDepartureAdmissionSync)
            {
                int facilityId = 0;
                foreach (KeyValuePair<int, uint> admission in ManagedDepartureAdmissions)
                {
                    if (admission.Value == parkedId)
                    {
                        facilityId = admission.Key;
                        break;
                    }
                }
                if (facilityId != 0)
                    ManagedDepartureAdmissions.Remove(facilityId);
            }
        }

        private static void ReleaseManagedDepartureAdmissionForVehicle(
            int facilityId,
            ushort vehicleId)
        {
            if (facilityId <= 0 || vehicleId == 0)
                return;
            lock (ManagedDepartureAdmissionSync)
            {
                uint token;
                if (ManagedDepartureAdmissions.TryGetValue(facilityId, out token)
                    && token == 65536u + vehicleId)
                {
                    ManagedDepartureAdmissions.Remove(facilityId);
                }
            }
        ...
        private static bool HandleNestedManagedRetrieval(
            ushort parkedId,
            out ManagedRetrievalState state)
        {
            state = default(ManagedRetrievalState);
            if (_managedRetrievalScopeDepth <= 0)
                return false;

            // Retrieval is synchronous on the simulation thread. A nested
            // owner/vanilla/TM:PE entry for the same identity participates in
            // the outer transaction without moving the parked record again.
            // A different identity cannot safely replace that transaction's
            // shared native-creation context, so leave it wholly native.
            if (parkedId == 0 || parkedId != _managedRetrievalScopeParkedId)
                return true;

            _managedRetrievalScopeDepth++;
            state = ManagedRetrievalState.CreateNested(parkedId);
            return true;
        }

        private static ManagedRetrievalState BeginManagedRetrievalScope(
            ushort parkedId,
            Vector3 originalPosition,
            Quaternion originalRotation)
        {
            _managedRetrievalScopeDepth = 1;
            _managedRetrievalScopeParkedId = parkedId;
            return new ManagedRetrievalState(
                parkedId,
                originalPosition,
                originalRotation,
                true);
        ...
        private static void CompleteManagedRetrievalScope(
            ManagedRetrievalState state,
            bool ownerBoundary)
        {
            if (!state.IsManaged)
                return;
            ...
            {
                if (_managedRetrievalScopeDepth > 0
                    && _managedRetrievalScopeParkedId == state.ParkedId)
                {
                    _managedRetrievalScopeDepth--;
                }
                return;
            ...
            if (ownerBoundary)
            {
                _pendingOwnerDeparture = default(ManagedDepartureState);
                _managedDepartureContext = default(ManagedDepartureContext);
                ReleaseManagedDepartureAdmission(state.ParkedId);
            }
            RestoreManagedRetrieval(state);
        }
            ...
            ushort expectedOwner)
        {
            ...
            simulationManager.AddAction(() =>
            {
                ...
                if (!EnsureManagedDepartureNativePath(
                        vehicleId,
                        ref vehicleData))
                {
                    TrackManagedDepartureStaging(vehicleId);
        ...
        private static bool EnsureManagedDepartureNativePath(
            ushort vehicleId,
            ref Vehicle vehicleData)
        {
            PathManager pathManager = PathManager.instance;
            ...
            if (pathId != 0u)
            {
                if (pathManager == null || pathId >= pathManager.m_pathUnits.m_size)
                    return false;
                ref PathUnit path = ref pathManager.m_pathUnits.m_buffer[pathId];
                if ((path.m_pathFindFlags & PathUnit.FLAG_FAILED) != 0)
                {
                    pathManager.ReleasePath(pathId);
                    vehicleData.m_path = 0u;
                    vehicleData.m_flags &= ~Vehicle.Flags.WaitingPath;
                    return false;
                }
                return (path.m_pathFindFlags & PathUnit.FLAG_READY) != 0;
            }
            ...
            PassengerCarAI ai = info == null
                ? null
                : info.m_vehicleAI as PassengerCarAI;
            if (!RestartPassengerCarPath(ai, vehicleId, ref vehicleData))
                return false;

            pathId = vehicleData.m_path;
            if (pathManager == null
                || pathId == 0u
                || pathId >= pathManager.m_pathUnits.m_size)
                return false;
            ref PathUnit requestedPath = ref pathManager.m_pathUnits.m_buffer[pathId];
            return (requestedPath.m_pathFindFlags & PathUnit.FLAG_READY) != 0
                   && (requestedPath.m_pathFindFlags & PathUnit.FLAG_FAILED) == 0;
        }

        private static void RequestManagedDepartureNativePath(
            ushort vehicleId,
            ref Vehicle vehicleData)
        {
            ...
            PassengerCarAI ai = info == null
                ? null
                : info.m_vehicleAI as PassengerCarAI;
            RestartPassengerCarPath(ai, vehicleId, ref vehicleData);
        }
        ...
        private static void ClearManagedDepartureTicket(ushort vehicleId)
        {
            if (vehicleId == 0)
                return;
            int facilityId = ManagedDepartureReleaseFacilities[vehicleId];
            ReleaseManagedDepartureAdmissionForVehicle(facilityId, vehicleId);
            ...
            ManagedDepartureReleaseWaitLogged[vehicleId] = false;
            ManagedDepartureAnimationQueued[vehicleId] = false;
            ManagedDepartureParkedIds[vehicleId] = 0;
            ManagedDepartureStagingTracked[vehicleId] = false;
        }
        ...
        private struct ManagedRetrievalState
        {
            ...
            public readonly bool OwnsScope;
            ...
            public static ManagedRetrievalState CreateNested(ushort parkedId)
            {
                return new ManagedRetrievalState(
                    parkedId,
                    Vector3.zero,
        ...
        private struct DeferredArrivalAssociation
        {
            public readonly ushort InstanceId;
            public readonly ushort ParkedId;
            public readonly ushort ArrivalVehicleId;
    ...
    internal static class UndergroundParkingLifecycleDiagnostics
    {
            ...
            string detail)
        {
            if (!UndergroundParkingGarageSettings.AdvancedDiagnostics
                || citizenId == 0u
                || _eventCount >= MaxLifecycleEvents)
                return;
