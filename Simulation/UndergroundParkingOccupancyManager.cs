...
namespace UndergroundParkingGarage
{
    internal struct UndergroundParkingCarVisual
    {
        ...
        public UndergroundParkingCarVisual(int facilityId, Vector3 position, Quaternion rotation, VehicleInfo info)
        {
    ...
    internal static class UndergroundParkingOccupancyManager
    {
        ...
        private static bool _warmupManaged;
        ...
        {
            UndergroundParkingScanCoordinator.CancelWarmup();
            Reservations.Clear();
            OccupiedSlots.Clear();
        ...
        public static byte[] SerializePersistentAssignments()
        {
            ...
            using (BinaryWriter writer = new BinaryWriter(stream))
            {
                ...
                foreach (KeyValuePair<ulong, SlotOccupancy> pair in OccupiedSlots)
                {
                    ...
                    UndergroundParkingBoundedBinaryString.ValidateForWrite(
                        info.name ?? string.Empty,
                        "UPG parked-assignment prefab identity");
                ...
                foreach (KeyValuePair<ushort, PendingVanillaRelease> pair in PendingVanillaReleases)
                {
                    ...
                    UndergroundParkingBoundedBinaryString.ValidateForWrite(
                        release.PrefabName ?? string.Empty,
                        "UPG pending vanilla-release prefab identity");
        ...
        public static void StagePersistentAssignments(byte[] data)
        {
            ...
            Dictionary<ushort, PendingVanillaRelease> stagedVanillaReleases =
                new Dictionary<ushort, PendingVanillaRelease>();
            HashSet<ushort> stagedParkedIds = new HashSet<ushort>();
            HashSet<ulong> stagedSlotKeys = new HashSet<ulong>();
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    int facilityId = reader.ReadInt32();
                    ...
                    if (parkedId == 0
                        || facilityId <= 0
                        || facilityId > ushort.MaxValue - 1
                        || slotIndex < 0
                        || string.IsNullOrEmpty(prefabName)
                        || !stagedParkedIds.Add(parkedId)
                        || !stagedSlotKeys.Add(MakeSlotKey(facilityId, slotIndex)))
                    {
                        throw new InvalidDataException(
                            "UPG parked-assignment record is invalid or duplicated at index "
                            + i
                            + ".");
                    }
                ...
                if (version >= 2)
                {
                    ...
                    {
                        ushort parkedId = reader.ReadUInt16();
                        uint expectedOwnerCitizen = reader.ReadUInt32();
                        string prefabName = UndergroundParkingBoundedBinaryString.Read(
                            reader,
                            "UPG pending vanilla-release prefab identity");
                        PendingVanillaRelease release = new PendingVanillaRelease(
                            parkedId,
                            expectedOwnerCitizen,
                            prefabName,
                            reader.ReadUInt16(),
                            ReadVector(reader),
                            ReadVector(reader));
                        if (release.ParkedId == 0
                            || string.IsNullOrEmpty(release.PrefabName)
                            || !UndergroundParkingAccessManager.IsFinite(
                                release.ReferencePosition)
                            || !UndergroundParkingAccessManager.IsFinite(
                                release.LaneDirection)
                            || stagedParkedIds.Contains(release.ParkedId)
                            || stagedVanillaReleases.ContainsKey(release.ParkedId))
                        {
                            throw new InvalidDataException(
                                "UPG pending vanilla-release record is invalid or duplicated at index "
                                + i
                                + ".");
                        }
                        stagedVanillaReleases.Add(release.ParkedId, release);
                    }
                }
            }

            PendingPersistentAssignments.AddRange(stagedAssignments);
            foreach (KeyValuePair<ushort, PendingVanillaRelease> pair
                     in stagedVanillaReleases)
            {
                PendingVanillaReleases.Add(pair.Key, pair.Value);
            }
        }
            ...
            out Quaternion originalRotation)
        {
            ...
            // The assignment owns the hidden record's rollback pose. Do not
            // capture the record's current transient pose: nested vanilla and
            // TM:PE retrieval entry points can observe it after an enclosing
            // prefix has already moved it to the pavement, and a failed native
            // attempt can leave a zero/invalid rotation in the live record.
            // Re-resolve the immutable assigned slot so every enclosing
            // finalizer restores the same validated underground destination.
            ulong slotKey;
            SlotOccupancy occupancy;
            if (!ParkedVehicleSlots.TryGetValue(parkedId, out slotKey)
                ...
                || occupancy.FacilityId != facility.Id
                || occupancy.SlotIndex < 0
                || occupancy.SlotIndex >= GetManagedSlotCapacity(facility))
            {
                return false;
            }

            originalPosition = GetUndergroundParkingSlotPosition(
                facility,
                occupancy.SlotIndex);
            originalRotation = GetUndergroundParkingSlotRotation(
                facility,
                occupancy.SlotIndex);
            if (!UndergroundParkingAccessManager.IsFinite(originalPosition)
                || !UndergroundParkingAccessManager.IsFinite(originalRotation))
            {
                UndergroundParkingLog.Error(
                    "UPG refused managed retrieval because its assigned garage-slot pose was invalid: parked="
                    + parkedId
                    + " facility="
                    + facility.Id
                    + " slot="
        ...
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static void GetFacilitySpaceCounts(
            UndergroundParkingFacility facility,
            out int parked,
            out int incoming)
        {
            ...
            foreach (KeyValuePair<ulong, SlotOccupancy> pair in OccupiedSlots)
            {
                if (pair.Value.FacilityId != facility.Id)
                    continue;
        ...
        private static void BeginWarmup()
        {
            ...
            _warmupNextParkedId = 1;
            _warmupManaged =
                UndergroundParkingScanCoordinator.TryQueueParkedVehicleWarmup(
                    StepManagedParkedVehicleCacheWarmup);
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        internal static bool StepManagedParkedVehicleCacheWarmup()
        {
            WarmupManagedParkedVehicleCache(1);
            return !_warmupActive;
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        internal static void CompleteManagedWarmupScan()
        {
            _warmupManaged = false;
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        internal static void UseLocalWarmupFallback()
        {
            _warmupManaged = false;
        }
        ...
        private static bool MoveParkedVehicle(
            ushort parkedId,
            ref VehicleParked parkedData,
            Vector3 position,
            Quaternion rotation,
