...
namespace UndergroundParkingGarage
{
    public static class UndergroundParkingRegistry
    {
        ...
        private const float MaximumSerializedGarageDimension = 512f;
        ...
        internal static string DescribeFacilityActivity(int facilityId)
        {
            if (facilityId <= 0)
                return "none";

            List<string> owners = new List<string>();
            if (UndergroundParkingEntryRouteManager.HasActivityForFacility(facilityId))
                owners.Add("road-route");
            if (UndergroundParkingPortalAnimationManager.HasActivityForFacility(facilityId))
                owners.Add("portal-animation");
            if (UndergroundParkingVisualManager.HasInternalParkingJourneyForFacility(facilityId))
                owners.Add("internal-journey");
            if (UndergroundParkingOccupancyHarmony.HasLifecycleActivityForFacility(facilityId))
                owners.Add("lifecycle-transaction");
            if (UndergroundParkingOccupancyManager.HasTransientActivityForFacility(facilityId))
                owners.Add("reservation-or-pending-slot");
        ...
        public static bool TryRemoveForTargetBuilding(ushort buildingId, out string status)
        {
            ...
            if (occupied > 0)
            {
                ...
                UndergroundParkingLog.Advanced(
                    "UPG attached-garage deletion blocked: facility=" + facility.Id
                    + " targetBuilding=" + buildingId
                    + " parked=" + occupied
                    + " activity=" + DescribeFacilityActivity(facility.Id));
                return false;
            }

            if (HasFacilityActivity(facility.Id))
            {
                string activity = DescribeFacilityActivity(facility.Id);
                status = "Cannot delete yet: this empty garage has an active parking reservation or vehicle transaction. Turn it off and allow that traffic to clear.";
                UndergroundParkingLog.Advanced(
                    "UPG attached-garage deletion blocked: facility=" + facility.Id
                    + " targetBuilding=" + buildingId
                    + " parked=0 activity=" + activity);
                return false;
            }
        ...
        public static int Restore(byte[] data)
        {
            ...
            HashSet<ushort> restoredEntranceBuildings = new HashSet<ushort>();
            HashSet<ushort> restoredTargetBuildings = new HashSet<ushort>();
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (id <= 0
                        ...
                        || surfaceSegmentId == 0
                        || surfaceSegmentPosition < 0f
                        || surfaceSegmentPosition > 1f
                        ...
                        || garageWidth <= 0f
                        || garageLength <= 0f
                        || garageWidth > MaximumSerializedGarageDimension
                        || garageLength > MaximumSerializedGarageDimension
                        || (version >= 11
                            && (garageDetailVariant < 0 || garageDetailVariant > 7))
                        || (version >= 3
                            && (targetBuildingId != 0
                    ...
                    {
                        throw new InvalidDataException(
                            "UPG facility record is invalid at index " + i + ".");
                    }
                    if (!restoredIds.Add(id))
                    {
                        throw new InvalidDataException(
                            "UPG facility ID is duplicated: " + id);
                    }
                    if (entranceBuildingId != 0
                        && !restoredEntranceBuildings.Add(entranceBuildingId))
                    {
                        throw new InvalidDataException(
                            "UPG entrance building is duplicated: " + entranceBuildingId);
                    }
                    if (targetBuildingId != 0
                        && !restoredTargetBuildings.Add(targetBuildingId))
                    {
                        throw new InvalidDataException(
                            "UPG attached host is duplicated: " + targetBuildingId);
                    }
                    ...
                    UndergroundParkingFacility restored = new UndergroundParkingFacility(
                        ...
                        garageDetailVariant);
                    if (restored.FloorCount != floorCount
                        || floorCount < UndergroundParkingGeometry.GetMinimumFloorCount(restored)
                        || floorCount > UndergroundParkingGeometry.GetMaximumFloorCount(restored))
                    {
                        throw new InvalidDataException(
                            "UPG facility floor count is invalid: id="
                            + id
                            + " floors="
                            + floorCount);
                    }
                    for (int restoredIndex = 0;
                         restoredIndex < restoredFacilities.Count;
                         restoredIndex++)
                    {
                        UndergroundParkingFacility prior =
                            restoredFacilities[restoredIndex];
                        if (IsSameRegisteredBuilding(prior, restored)
                            || IsSameTargetBuilding(prior, restored)
                            || IsSamePlacement(prior, restored))
                        {
                            throw new InvalidDataException(
                                "UPG facility placement is duplicated: id="
                                + id
                                + " prior="
                                + prior.Id);
                        }
            ...
            List<ImportedParkedAssignment> importedAssignments)
        {
            ...
            HashSet<ushort> importedTargetBuildings = new HashSet<ushort>();
            HashSet<ushort> importedParkedIds = new HashSet<ushort>();
            HashSet<ulong> importedSlotKeys = new HashSet<ulong>();
            HashSet<int> rebuildFacilityIds = new HashSet<int>();
            HashSet<ushort> rebuildParkedIds = new HashSet<ushort>();
            HashSet<ulong> rebuildSlotKeys = new HashSet<ulong>();
            for (int assignmentIndex = 0;
                 assignmentIndex < importedAssignments.Count;
                 assignmentIndex++)
            {
                ImportedParkedAssignment assignment =
                    importedAssignments[assignmentIndex];
                if (assignment.ParkedId != 0)
                    importedParkedIds.Add(assignment.ParkedId);
                if (assignment.FacilityId > 0 && assignment.SlotIndex >= 0)
                {
                    importedSlotKeys.Add(
                        MakeImportedSlotKey(
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(stream))
                {
                    ...
                    {
                    int rebuildFacilityId = reader.ReadInt32();
                    ...
                    int rebuildSpacesPerFloor = reader.ReadInt32();
                    byte operatingState = reader.ReadByte();

                    UndergroundParkingFacility existing;
                    int activeFacilityId = 0;
                    if (rebuildFacilityId <= 0
                        || !rebuildFacilityIds.Add(rebuildFacilityId))
                    {
                        throw new InvalidDataException(
                            "UPG rebuild facility identity is invalid or duplicated: "
                            + rebuildFacilityId);
                    }
                    if (kind != 1 && kind != 2)
                    {
                        throw new InvalidDataException(
                            "UPG rebuild facility kind is unsupported: " + kind);
                    }
                    bool attachedRecord = kind == 2;
                    bool invalidDefinition = surfaceSegmentId == 0
                            || surfaceSegmentPosition < 0f
                            || surfaceSegmentPosition > 1f
                            || !IsFinite(surfaceSegmentPosition)
                            || !IsFinite(surfaceRoadPosition)
                            ...
                            || garageLength > MaximumSerializedGarageDimension
                            || floorCount < 1
                            || floorCount > UndergroundParkingGeometry.MaxFloorCount
                            || rebuildSpacesPerFloor <= 0
                            || rebuildSpacesPerFloor > maximumRecords
                            || operatingState > 3
                            || (attachedRecord
                                ? targetBuildingId == 0 || entranceBuildingId != 0
                                : entranceBuildingId == 0 || targetBuildingId != 0);
                    if (invalidDefinition)
                    {
                        throw new InvalidDataException(
                            "UPG rebuild facility record is invalid at index "
                            + index
                            + ".");
                    }
                    ...
                    if (attachedRecord
                        && !importedTargetBuildings.Add(targetBuildingId))
                    {
                        throw new InvalidDataException(
                            "UPG rebuild attached host is duplicated: "
                            + targetBuildingId);
                    }
                    ...
                    int rebuildCapacity = rebuildSpacesPerFloor * floorCount;
                    if (storedCount < 0
                        || storedCount > maximumRecords
                        || storedCount > rebuildCapacity)
                    ...
                    for (int storedIndex = 0; storedIndex < storedCount; storedIndex++)
                    {
                        ...
                        string prefabName = UndergroundParkingBoundedBinaryString.Read(
                            reader,
                            "UPG rebuild stored prefab identity");
                        byte vehicleKind = reader.ReadByte();
                        ...
                        if (vehicleKind != 1 && vehicleKind != 2)
                        {
                            throw new InvalidDataException(
                                "UPG rebuild stored vehicle kind is unsupported: "
                                + vehicleKind);
                        }
                        if (recordKind != 2)
                        {
                            throw new InvalidDataException(
                                "UPG rebuild stored vehicle is not a parked identity.");
                        }
                        if (parkedId == 0 || slot < 0 || string.IsNullOrEmpty(prefabName))
                        {
                            throw new InvalidDataException(
                                "UPG rebuild stored vehicle record is invalid at index "
                                + storedIndex
                                + ".");
                        }
                        ulong rebuildSlotKey = MakeImportedSlotKey(
                            rebuildFacilityId,
                            slot);
                        if (!rebuildParkedIds.Add(parkedId)
                            || !rebuildSlotKeys.Add(rebuildSlotKey))
                        {
                            throw new InvalidDataException(
                                "UPG rebuild stored assignment is duplicated: parked="
                                + parkedId
                                + " facility="
                                + rebuildFacilityId
                                + " slot="
                        ...
                        {
                            ulong slotKey = MakeImportedSlotKey(
                                activeFacilityId,
                                slot);
                            if (!importedParkedIds.Add(parkedId)
                                || !importedSlotKeys.Add(slotKey))
                            {
                                throw new InvalidDataException(
                                    "UPG rebuild parked assignment is duplicated: parked="
                                    + parkedId
                                    + " facility="
                                    + activeFacilityId
                                    + " slot="
        ...
        private static ulong MakeImportedSlotKey(int facilityId, int slotIndex)
        {
            return ((ulong)(uint)facilityId << 32) | (uint)slotIndex;
        }
