...
namespace UndergroundParkingGarage
{
    ...
    internal sealed class UndergroundParkingLaneLayout
    {
        ...
        private const float MinimumTunnelWallCrossingAlignment = 0.5f;
        private const float TunnelWallAlignmentScoreWeight = 0.25f;
        private const float CompactAttachedEdgeClearance = 0.6f;
        private const float PackedAttachedEdgeClearance = 0.4f;
        ...
        public readonly bool UsesCompactAttachedLayout;
        ...
        private UndergroundParkingLaneLayout(
            ...
            bool usesCompactAttachedLayout = false)
        {
            Bays = bays;
            ...
            UsesCompactAttachedLayout = usesCompactAttachedLayout;
        }
            ...
            out UndergroundParkingLaneLayout layout)
        {
            ...
            bool selectedUsesCompactAttachedLayout = false;
            ...
            else for (int i = 0; i < candidates.Length; i++)
            {
                ...
                float candidateWallCrossingAlignment;
                if (!TryResolveRampMouth(
                        ...
                        out candidateGrade,
                        out candidateWallCrossingAlignment))
                {
                    continue;
                }
                ...
                float excessiveGradePenalty = candidateGrade > 0.25f
                    ...
                    : 0f;
                float wallAlignmentPenalty =
                    (1f - candidateWallCrossingAlignment)
                    * TunnelWallAlignmentScoreWeight;
                float score = gradeError
                              + excessiveGradePenalty
                              + wallAlignmentPenalty;
                bool feasible = candidateGrade <= 0.25f
                                && candidateWallCrossingAlignment
                                   >= MinimumTunnelWallCrossingAlignment
            ...
            // A footprint that cannot retain the complete logical grid in the
            // established module family receives a bounded physical-layout
            // search. All variants keep credible bay pitch and clear driving
            // aisles. The entrance-side-cross-aisle variants let the tunnel
            // land without consuming bays; a sole double-loaded aisle needs no
            // cross aisle and accepts the tunnel directly at its end.
            if (!foundCandidate && !standalone)
            {
                float packedWidth = usableWidth
                                    + (UndergroundParkingGeometry.ParkingSlotEdgePadding
                                       - PackedAttachedEdgeClearance) * 2f;
                float packedLength = usableLength
                                     + (UndergroundParkingGeometry.ParkingSlotEdgePadding
                                        - PackedAttachedEdgeClearance) * 2f;
                LayoutCandidate[] compactCandidates =
                {
                    BuildCandidate(packedWidth, packedLength, true, 1, false, CrossAisleWidth,
                        requiredBays, true, true),
                    BuildCandidate(packedWidth, packedLength, true, -1, false, CrossAisleWidth,
                        requiredBays, true, true),
                    BuildCandidate(packedWidth, packedLength, false, 1, false, CrossAisleWidth,
                        requiredBays, true, true),
                    BuildCandidate(packedWidth, packedLength, false, -1, false, CrossAisleWidth,
                        requiredBays, true, true),
                    BuildThreeRowAttachedCandidate(usableWidth, usableLength, true, 1, requiredBays),
                    BuildThreeRowAttachedCandidate(usableWidth, usableLength, true, -1, requiredBays),
                    BuildThreeRowAttachedCandidate(usableWidth, usableLength, false, 1, requiredBays),
                    BuildThreeRowAttachedCandidate(usableWidth, usableLength, false, -1, requiredBays),
                    BuildSingleAisleAttachedCandidate(usableWidth, usableLength, true, 1, requiredBays),
                    BuildSingleAisleAttachedCandidate(usableWidth, usableLength, true, -1, requiredBays),
                    BuildSingleAisleAttachedCandidate(usableWidth, usableLength, false, 1, requiredBays),
                    BuildSingleAisleAttachedCandidate(usableWidth, usableLength, false, -1, requiredBays)
                };
                for (int i = 0; i < compactCandidates.Length; i++)
                {
                    LayoutCandidate candidate = compactCandidates[i];
                    if (candidate.Capacity < requiredBays)
                        continue;
                    ...
                    selectedScore = score;
                    selectedFeasible = feasible;
                    selectedUsesCompactAttachedLayout = true;
                    foundCandidate = true;
                }
            }
            if (!foundCandidate)
            ...
            float rampTopAlong;
            float selectedGrade;
            float selectedWallCrossingAlignment;
            if (!TryResolveRampMouth(
                    facility,
                    localEntrance,
                    selected,
                    ...
                    out rampTopAlong,
                    out selectedGrade,
                    out selectedWallCrossingAlignment))
            {
                return false;
            }

            if (selectedUsesCompactAttachedLayout)
            {
                List<UndergroundParkingBay> compactStableBays =
                    new List<UndergroundParkingBay>(requiredBays);
                for (int slot = 0; slot < requiredBays; slot++)
                    compactStableBays.Add(allBays[slot]);

                Vector3 compactRampTopPosition = selected.AislesAlongForward
                    ? new Vector3(rampTopAcross, 0f, rampTopAlong)
                    : new Vector3(rampTopAlong, 0f, rampTopAcross);
                Vector3 compactIngressPosition = selected.AislesAlongForward
                    ? new Vector3(ingressAisle, 0f, selected.CrossAisleCoordinate)
                    : new Vector3(selected.CrossAisleCoordinate, 0f, ingressAisle);
                layout = new UndergroundParkingLaneLayout(
                    compactStableBays,
                    new List<UndergroundParkingBay>(allBays),
                    selected.BayWidth,
                    selected.AislesAlongForward,
                    selected.CrossAisleCoordinate,
                    selected.CrossAisleSpan,
                    selected.EntranceSign,
                    compactRampTopPosition,
                    compactIngressPosition,
                    selectedFeasible,
                    true);
                return true;
            }
        ...
        private static bool TryResolveRampMouth(
            ...
            out float wallCrossingAlignment)
        {
            rampTopAcross = 0f;
            ingressAisle = 0f;
            rampTopAlong = 0f;
            tunnelGrade = float.MaxValue;
            wallCrossingAlignment = 0f;
            ...
            // An external approach owns a square lower landing whose near,
            // left or right face is selected independently; a side approach
            // there is intentional. Only a tunnel terminating directly at an
            // internal garage wall must cross that wall rather than run along
            // it and collapse the framed mouth into a flattened taper.
            if (externalApproach)
            {
                wallCrossingAlignment = 1f;
                return true;
            }
            Vector2 tunnelDirection = garageTunnel - surfaceTunnel;
            if (tunnelDirection.sqrMagnitude > 0.001f)
            {
                tunnelDirection.Normalize();
                wallCrossingAlignment = Mathf.Abs(
                    Vector2.Dot(tunnelDirection, outward));
            }
            return true;
        }
        ...
        private static LayoutCandidate BuildCandidate(
            ...
            bool crossAisleAtEntrance = false)
        {
            ...
            // Parking is enumerated away from the entrance. Established
            // layouts place their cross aisle at the far end and reserve the
            // real ramp pair later; compact fallbacks place the cross aisle at
            // the entrance and therefore keep every generated bay.
            float parkingStart = entranceSign > 0
                ? alongExtent * 0.5f
                  - (crossAisleAtEntrance ? crossAisleSpan : 0f)
                  - bayWidth * 0.5f
                : -alongExtent * 0.5f
                  + (crossAisleAtEntrance ? crossAisleSpan : 0f)
                  + bayWidth * 0.5f;
        ...
        private static LayoutCandidate BuildThreeRowAttachedCandidate(
            float usableWidth,
            float usableLength,
            bool aislesAlongForward,
            int entranceSign,
            int requiredCapacity)
        {
            float crossExtent = (aislesAlongForward ? usableWidth : usableLength)
                                + (UndergroundParkingGeometry.ParkingSlotEdgePadding
                                   - CompactAttachedEdgeClearance) * 2f;
            float alongExtent = (aislesAlongForward ? usableLength : usableWidth)
                                + (UndergroundParkingGeometry.ParkingSlotEdgePadding
                                   - CompactAttachedEdgeClearance) * 2f;
            float threeRowSpan = BayDepth * 3f + AisleWidth * 2f;
            if (crossExtent + 0.01f < threeRowSpan)
                return default(LayoutCandidate);

            int alongCount = Mathf.CeilToInt(requiredCapacity / 3f);
            float parkingRun = alongExtent - CrossAisleWidth;
            float bayWidth = parkingRun / Mathf.Max(1, alongCount);
            if (alongCount <= 0 || bayWidth < 2.3f || bayWidth > 3.5f)
                return default(LayoutCandidate);

            float bayToAisle = BayDepth * 0.5f + AisleWidth * 0.5f;
            float firstAisle = -bayToAisle;
            float secondAisle = bayToAisle;
            float crossAisleCoordinate = entranceSign
                                         * (alongExtent * 0.5f
                                            - CrossAisleWidth * 0.5f);
            float parkingStart = entranceSign > 0
                ? alongExtent * 0.5f - CrossAisleWidth - bayWidth * 0.5f
                : -alongExtent * 0.5f + CrossAisleWidth + bayWidth * 0.5f;
            float parkingDirection = entranceSign > 0 ? -1f : 1f;

            List<UndergroundParkingBay> bays =
                new List<UndergroundParkingBay>(alongCount * 3);
            for (int along = 0; along < alongCount; along++)
            {
                float alongPosition = parkingStart
                                      + parkingDirection * along * bayWidth;
                AddCompactAttachedBay(
                    bays,
                    aislesAlongForward,
                    firstAisle - bayToAisle,
                    firstAisle,
                    alongPosition,
                    -1f);
                AddCompactAttachedBay(
        ...
        private static LayoutCandidate BuildSingleAisleAttachedCandidate(
            float usableWidth,
            float usableLength,
            bool aislesAlongForward,
            int entranceSign,
            int requiredCapacity)
        {
            ...
            float alongExtent = (aislesAlongForward ? usableLength : usableWidth)
                                + (UndergroundParkingGeometry.ParkingSlotEdgePadding
                                   - CompactAttachedEdgeClearance) * 2f;
            float moduleSpan = BayDepth * 2f + AisleWidth;
            if (crossExtent + 0.01f < moduleSpan)
                return default(LayoutCandidate);

            int alongCount = Mathf.CeilToInt(requiredCapacity / 2f);
            float bayWidth = alongExtent / Mathf.Max(1, alongCount);
            if (alongCount <= 0 || bayWidth < 2.3f || bayWidth > 3.5f)
                return default(LayoutCandidate);

            float bayToAisle = BayDepth * 0.5f + AisleWidth * 0.5f;
            float parkingStart = entranceSign
                                 * (alongExtent * 0.5f - bayWidth * 0.5f);
            float parkingDirection = entranceSign > 0 ? -1f : 1f;
            List<UndergroundParkingBay> bays =
                new List<UndergroundParkingBay>(alongCount * 2);
            for (int along = 0; along < alongCount; along++)
            {
                float alongPosition = parkingStart
                                      + parkingDirection * along * bayWidth;
            ...
            float directIngressCoordinate = entranceSign
                                            * (alongExtent * 0.5f
                                               - (TunnelGarageWallOverlap
                                                  - CompactAttachedEdgeClearance));
            return new LayoutCandidate(
                bays,
                bayWidth,
                aislesAlongForward,
                directIngressCoordinate,
                0f,
                entranceSign,
                alongExtent);
        }

        private static void AddCompactAttachedBay(
            List<UndergroundParkingBay> bays,
            bool aislesAlongForward,
            float bayAcross,
            float aisleAcross,
            float alongPosition,
            float parkingDirection)
        {
            Vector3 bayPosition;
            Vector3 lanePosition;
            Vector3 parkingDirectionVector;
            if (aislesAlongForward)
            {
                bayPosition = new Vector3(bayAcross, 0f, alongPosition);
                lanePosition = new Vector3(aisleAcross, 0f, alongPosition);
                parkingDirectionVector = new Vector3(parkingDirection, 0f, 0f);
            }
            else
            {
                bayPosition = new Vector3(alongPosition, 0f, bayAcross);
                lanePosition = new Vector3(alongPosition, 0f, aisleAcross);
                parkingDirectionVector = new Vector3(0f, 0f, parkingDirection);
            }
            bays.Add(new UndergroundParkingBay(
                bayPosition,
                lanePosition,
