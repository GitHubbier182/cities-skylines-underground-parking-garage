...
Local version 2.4.1 retains the released v2.4.0 functionality and corrects the
remaining TM:PE compatibility boundaries. Once the exact onward
pedestrian continuation has a real or waiting path, UPG immediately restores
the citizen's normal parked-car association. It no longer waits for that path
to become idle, where a reused citizen instance could make the restoration
occur at an unrelated later destination and disrupt that car's parking retry.
Nested vanilla and TM:PE retrieval calls now share one outer parked-car
virtualization transaction. Every unsuccessful attempt restores the exact
identity to its authoritative assigned garage slot rather than capturing an
already-virtualized or invalid transient record pose, preventing the sustained
invalid-pose retry loop reported against v2.4.0.
Version 2.4.0 limited
garage presentation work to facilities visible on camera below 500 metres.
TM:PE-controlled cars still retain their exact vehicle and occupants and follow
their complete accepted native road route to the validated entrance;
an off-camera or high-camera arrival then commits directly to its reserved
underground space without allocating a presentation proxy or portal-animation
queue entry. Visible arrivals keep the complete entrance and internal journey.
With TM:PE realistic parking either enabled or disabled, the entrance proxy
begins at the native car's last actually rendered position and heading and
continues along one non-zero road tangent into the perpendicular entrance curve,
without a presentation stop or snap-turn. A following car may advance as soon
as the preceding vehicle's rear has geometrically cleared the entrance; it no
longer waits for that presentation to reach its final underground handoff. The
first car's visible descent and realistic internal parking journey continue.
Managed departures continue automatically after the exact car has been
initialized and staged; UPG never changes the player's selected game speed.
The neutral x-ray vehicle owns the complete departure from its bay through the
internal aisles and rendered tunnel ramp to the upper chamber; the exact-colour
surface vehicle takes over only there for the perpendicular turn onto the road.
Entering neutral vehicles choose the smaller live yaw at internal direction
changes, with a right turn used only when the two half-turns are exactly equal.
The passenger-car simulation hook now rejects vehicles without a UPG route
before pose interpolation, and dormant advanced diagnostics perform no lifecycle
sampling or occupancy enumeration. Garage structures and every committed parked
car remain persistently visible when their full bounds intersect the
Transport/x-ray camera, including at wide zoom; off-screen facilities submit no
underground draw work. Moving presentation proxies additionally retain the
500-metre camera-height gate. Version 2.3.0 restored the accepted zero-facility isolation, first-garage
cache and live-pose identity corrections over the published v2.2.0 baseline.
Loaded-city and explicit rebuild parked-vehicle cache discovery now cooperates
with Scratchy's Scan Manager on the simulation thread, one parked ID per step,
with the same cursor continuing through UPG's local paced fallback if the shared
broker is unavailable. Live traffic, reservations, portal transactions, camera
visibility and animation remain outside the shared queue so scheduling cannot
change their timing or ownership.
...
ledger. Version 2.4.1 is published on Steam Workshop item `3763687411`; this
repository is its matching clean public-source snapshot, tagged `v2.4.1`.
...
that left-to-right order. Its header is a square standalone sign with a thin
white perimeter, blue parking field and proportionally tall white `P`; the
detailed entrance illustrations remain on the four placement tiles.
...
portal.
With realistic parking disabled, an ordinary UPG route begins only from the
car's exact live road lane and follows that lane's direction to the connected
entrance. The native both-directions detailed source is reserved for a proven
importing/exporting outside-connection handoff; a normal city car whose live
road source cannot be established remains wholly vanilla-owned for that
parking attempt.
For retrieval, the cim and exact parked identity remain at the validated
pavement exit until the departure direction and a buffered road release are
available. An arrival may use its separate entrance side at the same time. The
original TM:PE/vanilla parked-car entry then
creates the exact real car once; UPG holds it through the visible internal and
surface departure and publishes it only after its native passenger-car path is
ready. A failed native path is retried without releasing the car or occupants.
...
After a completed garage arrival, the temporary parked-car quarantine belongs
only to that exact arrival car and onward pedestrian continuation. As soon as
that continuation owns a real or waiting path—or its instance or active vehicle
is replaced—TM:PE regains the citizen's normal parked-car ownership. UPG does
not wait for the path's later idle state, carry the hidden association through
an unrelated trip or trigger a distant garage retrieval when that trip ends.
...
entrance has independent arrival and departure surface owners. While the
arrival owner is busy, UPG defers the next arriving car's terminal parking
callback before marking the
...
Building-attached entrances use one ground solution for the complete visible
surface. The road-facing edge meets the live road's exact yaw, pitch and roll,
and is the sole road-owned row. Every following row of the dense tarmac grid
uses its own direct detailed-terrain sample plus the shared surface clearance;
the first triangle strip joins the exact road seam to that ground without
placing interpolated rows below an uphill plot or relying on a hidden relative
datum.
Both side kerbs, the blue-and-white `P`, roadside sign, light and tunnel mouth
use that same contour, so a sloping plot cannot bury the entrance or leave a
flat floating slab.
...
resulting grade. A wall reached almost parallel to its face is rejected as a
collapsed opening rather than being allowed to win on grade alone; an
irregular footprint therefore uses the logical side wall that the tunnel
actually crosses. The wall opposite the entrance remains valid when the footprint
...
If none of the four walls can provide a credible wall crossing at or below a 25% grade,
...
acknowledged. When the entrance is free, that current pose requests native
unspawn on the first proxy update instead of imposing a confirmation-cycle
dwell; the proxy remains hidden until acknowledgement, so neither an earlier
FIFO-entry position, a stale request-time sample, a duplicate body nor a
motorcycle snapping upright can create a handoff step. Both
...
that boundary participates in the same outer identity scope around TM:PE's one
native vehicle creation; nested owner, vanilla and TM:PE callbacks never move
the parked record a second time or replace the rollback pose. The rollback pose
is always regenerated from the assignment's validated facility and slot, not
copied from the temporarily published live record. The initialized real car is
then held behind the shared departure
...
route. A request that creates no vehicle restores the underground record to
that exact slot and changes neither occupancy nor native lifecycle state.
...
logical capacity and slot identities stay unchanged. They retain their feasible
tunnel-selected aisle and may evenly tighten only their bay pitch, to no less
than 2.3 metres, when fixed spacing would otherwise lose a partial boundary row.
Only when every standard full double-loaded candidate is too small, a bounded
compact attached selector compares packed complete modules, a three-row
double/single-loaded arrangement and a sole double-loaded aisle. Multi-aisle
variants place the cross aisle at the entrance so the tunnel landing removes no
bay; a sole aisle accepts direct tunnel ingress at its end. All retain a minimum
2.3-metre bay pitch and complete 5.5-metre circulation aisles. Existing
successful attached layouts are unchanged, and kiosk fitting remains isolated
to the kiosk's approved axis and midpoint aisle. Each
...
validated entrance gate, road-to-perpendicular exact-colour entrance animation,
...
Parking Management reports cars already parked and incoming reserved spaces as
separate counts against the same capacity. An incoming claim is not a free bay:
the exact active car owns it while following its accepted route or completing
the garage handoff. Closing a garage blocks new arrivals but does not create
departures; parked cars leave when vanilla or Real Time schedules their owners'
next journeys.
...
- Current facility data, parked assignments and the legacy rebuild snapshot are restored transactionally. Invalid identities, duplicate facilities, parked records or slots, truncated fields, oversized geometry and unbounded prefab strings reject their complete source without publishing partial state.
...
- The `P` tab performs placement and refresh work only while its exact page is selected; advanced diagnostics separately attribute its closed, open and post-close callback windows.
...
- Active work and backlog: `ISSUES_AND_PLANS.md`
- Ownership ledger: `DESIGN_PRINCIPLES.md`
- Workshop dependencies: `WORKSHOP_DEPENDENCIES.md`
- Historical issue and release records: `Archive/`
