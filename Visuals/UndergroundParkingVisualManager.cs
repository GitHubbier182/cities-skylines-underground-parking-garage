...
namespace UndergroundParkingGarage
{
    internal static class UndergroundParkingVisualManager
    {
        ...
        private const float BuildingAttachedKerbSurfaceLift =
            BuildingAttachedPortalSurfaceLift + 0.01f;
            ...
            Color surfaceColor)
        {
            if (info == null
                ...
                || !ShouldAnimateInternalParkingJourney(facility))
            {
                return false;
            }
            ...
            UndergroundParkingFacility facility)
        {
            ...
            Vector3 centre = UndergroundParkingGeometry.ResolveCurrentVisualGarageCenter(facility);
            Vector3 midpoint = Vector3.Lerp(
                facility.EntrancePosition,
                centre,
                0.5f);
            if (!IsPointInsideCameraEnvelope(camera, facility.EntrancePosition)
                && !IsPointInsideCameraEnvelope(camera, facility.VehicleNodePosition)
                && !IsPointInsideCameraEnvelope(camera, midpoint)
                && !IsPointInsideCameraEnvelope(camera, centre))
            {
                return false;
            }
            Vector3 cameraDelta = camera.transform.position - centre;
            return Mathf.Abs(cameraDelta.y) <= InternalJourneyMaximumCameraHeight;
        }
        ...
        private static void UpdateInternalParkingJourneys()
        {
            ...
            for (int i = InternalJourneys.Count - 1; i >= 0; i--)
            {
                ...
                Vector3 proposedPosition = journey.GetPositionAtDistance(proposedDistance);
                bool aisleBlocked = false;
                for (int otherIndex = 0; otherIndex < InternalJourneys.Count; otherIndex++)
                {
                    if (otherIndex == i)
                        continue;
                    InternalParkingJourney other = InternalJourneys[otherIndex];
                    if (other.FacilityId == journey.FacilityId
                        && other.Sequence < journey.Sequence
                        && (other.Position - proposedPosition).sqrMagnitude < 12.25f)
                    {
                        aisleBlocked = true;
                        break;
                    }
                ...
                for (int otherIndex = 0;
                     !aisleBlocked && otherIndex < InternalDepartureJourneys.Count;
                     otherIndex++)
                {
                    InternalParkingJourney other =
                        InternalDepartureJourneys[otherIndex].Movement;
                    if (other.FacilityId == journey.FacilityId
                        && other.Sequence < journey.Sequence
                        && (other.Position - proposedPosition).sqrMagnitude < 12.25f)
                    {
        ...
        private static void UpdateInternalDepartureJourneys()
        {
            ...
            for (int i = InternalDepartureJourneys.Count - 1; i >= 0; i--)
            {
                ...
                if (UndergroundParkingPortalAnimationManager.QueueDeparture(
                        departure.Info,
                        departure.Facility,
            ...
            UndergroundParkingFacility facility)
        {
            ...
            UndergroundParkingLog.Advanced("Built building-attached flat entrance surface: facility="
                                        ...
                                        + " profileAnchor=live-road-plane"
                                        + " raisedRoadTerrainRejected=True"
                                        + " surfaceClearance=0.06"
                                        + " sideKerbs=viewer-left-right-only"
                                        + " kerbSurfaceClearance=0.07"
            ...
            out Vector3 garageEntry)
        {
            ...
            Vector3 surfaceLeftCarriageway = surfaceCenter
                                             ...
                                             + roadNormal * ResolveDetailedRoadProfileOffset(
                                                 ...
                                                 -surfaceHalfWidth, -surfaceHalfDepth)
            ...
            float garageRadius)
        {
            ...
            RenderItems.Add(new RenderItem(
                mesh, Matrix4x4.identity, GetGarageStructureMaterial(), center,
            ...
            UndergroundParkingLog.Advanced("UPG upward full-slab-mouth tunnel built: facility="
                                        ...
                                        + " singleRamp=True"
                                        + " surfaceMouthPlane=horizontal-road-slab"
            ...
            float localZ)
        {
            ...
            // Terrain far below or above the road belongs to a raised/sunken
            // network context and is not a usable pavement profile.
            if (Mathf.Abs(terrainCenterY - center.y) > 0.85f)
                return 0f;

            Vector3 planarSample = center
                                   + roadTangent * localX
                                   + inward * localZ;
            float terrainShape = terrain.SampleDetailHeight(planarSample)
                                 - terrainCenterY;
            float roadPlaneShape = planarSample.y - center.y;
            float verticalOffset = Mathf.Clamp(
                terrainShape - roadPlaneShape,
                -0.45f,
                0.45f);
            // Terrain detail describes the surrounding ground, not the rendered
            // road deck. Keep a local pavement profile without allowing it to
            // pull this surface through the road or visibly above it. With the
            // existing 0.06m lift the lower bound retains 0.04m clearance; the
            // kerbs keep their exact additional 0.01m.
            return Mathf.Clamp(
                verticalOffset / Mathf.Max(0.25f, roadNormal.y),
                -0.02f,
                0.12f);
        }
        ...
        private static DrawCallData RenderXrayVisuals(RenderManager.CameraInfo cameraInfo, string passName, bool forceVisible)
        {
            ...
            if (!forceVisible && !ShouldShowXrayVisuals())
                return drawCallData;
            ...
            Material material)
        {
            ...
            if (!forceVisible && !cameraInfo.CheckRenderDistance(position, 4096f))
                return 0;
        ...
        private static bool ShouldShowXrayVisuals()
        {
            ...
            {
                InfoManager manager = InfoManager.instance;
                return manager != null && manager.CurrentMode != InfoManager.InfoMode.None;
            }
            catch
            {
                return false;
        ...
        private struct RenderItem
        {
            ...
            public RenderItem(Mesh mesh, Matrix4x4 matrix, Material material, Vector3 center, float radius)
            {
                Mesh = mesh;
                Matrix = matrix;
                Material = material;
