...
namespace UndergroundParkingGarage
{
    internal static class UndergroundParkingPortalAnimationManager
    {
        ...
        private const float ArrivalEntranceRearClearance = 0.35f;
        private const int EntranceClearanceSamples = 64;
        private const float SurfaceDescentStartProgress = 0.32f;
        private const int PortalLengthSamples = 32;
        private const float MaximumAnimationCameraHeight = 500f;
        ...
        private static readonly HashSet<int> DepartureRepathFacilities =
            new HashSet<int>();
        // The entrance has two physical traffic sides. Each direction owns
        // only its own surface admission, so a following arrival waits for the
        // preceding arrival to clear but never waits for an opposing departure
        // or for the longer underground parking journey.
        private static readonly HashSet<int> ActiveArrivalFacilities =
            new HashSet<int>();
        private static readonly HashSet<int> ActiveDepartureFacilities =
            new HashSet<int>();
        ...
        private static PortalAnimationDriver _driver;
        ...
        private static int _arrivalClearanceLogCount;
        ...
        public static void Shutdown()
        {
            ...
            lock (Sync)
            {
                ...
                Pending.Clear();
                ActiveArrivalFacilities.Clear();
                ActiveDepartureFacilities.Clear();
        ...
        public static bool HasActivityForFacility(int facilityId)
        {
            ...
            lock (Sync)
            {
                return ActiveArrivalFacilities.Contains(facilityId)
                       || ActiveDepartureFacilities.Contains(facilityId);
            }
        }

        public static bool HasArrivalActivityForFacility(int facilityId)
        {
            if (facilityId <= 0)
                return false;
            lock (Sync)
        ...
        public static bool HasDepartureActivityForFacility(int facilityId)
        {
            if (facilityId <= 0)
                return false;
            lock (Sync)
                return ActiveDepartureFacilities.Contains(facilityId);
        }

        internal static int DrawXrayDepartures(Camera camera)
        {
            return _driver == null ? 0 : _driver.DrawXrayDepartures(camera);
        }
        ...
        public static bool QueueDeparture(
            ...
            Color surfaceColor)
        {
            return Queue(info, facility, connection, false, Vector3.zero, Quaternion.identity, surfaceColor, vehicleId, false, false);
        }

        public static bool QueueDepartureFromSurfacePortal(
            VehicleInfo info,
            UndergroundParkingFacility facility,
            UndergroundParkingRoadConnection connection,
            ushort vehicleId,
            Color surfaceColor)
        {
            return Queue(info, facility, connection, false, Vector3.zero, Quaternion.identity, surfaceColor, vehicleId, false, true);
        }
            ...
            out Vector3 portalControl)
        {
            ...
            // The final native render pose is the only surface handoff point.
            // Its forward tangent must remain non-zero even when that pose is
            // exactly level with the entrance centre: clamping this handle to
            // the road-longitudinal projection made a perpendicular entrance
            // produce a zero derivative, followed by a visible stop and an
            // abrupt diagonal twist on the next proxy frame.
            float startLead = Mathf.Min(
                3f,
                Mathf.Max(0.75f, surfaceDistance * 0.35f));
        ...
        private static float CalculateArrivalAdmissionReleaseProgress(
            VehicleInfo info,
            UndergroundParkingFacility facility,
            Vector3 start,
            Vector3 startControl,
            ...
            Vector3 underground)
        {
            Vector3 inward = portal - facility.VehicleNodePosition;
            inward.y = 0f;
            if (inward.sqrMagnitude <= 0.001f)
                inward = facility.Side;
            inward = NormalizeFlat(inward, portal - start);
            ...
            if (vehicleLength <= 0.1f)
                vehicleLength = 4f;
            float requiredCentreTravel = vehicleLength * 0.5f
                                         + ArrivalEntranceRearClearance;

            for (int sample = 1; sample <= EntranceClearanceSamples; sample++)
            {
                float progress = sample / (float)EntranceClearanceSamples;
                Vector3 position = EvaluateArrivalTraversalPath(
                    start,
                    startControl,
                    portalControl,
                    portal,
                    underground,
                    progress);
                Vector3 fromEntrance = position - facility.VehicleNodePosition;
                fromEntrance.y = 0f;
                if (Vector3.Dot(fromEntrance, inward) >= requiredCentreTravel)
                    return progress;
            }

            // Unusually long vehicles or malformed geometry retain the
            // established completion boundary rather than releasing early.
            return 1f;
        }
            ...
            bool surfaceOnlyDeparture)
        {
            ...
            Vector3 traversalEnd = !arrival && surfaceOnlyDeparture
                ? portal
                : arrivalEnd;
            float duration = CalculateTraversalDuration(
                ...
                portal,
                traversalEnd);
            float arrivalAdmissionReleaseProgress = arrival
                ? CalculateArrivalAdmissionReleaseProgress(
                    info,
                    facility,
                    lane,
                    arrivalApproach,
            ...
            PortalAnimationRequest request = arrival
                ? new PortalAnimationRequest(info, lane, arrivalApproach, portal, arrivalEnd, true, arrivalRotation, surfaceColor, facility.Id, vehicleId, facility.TargetBuildingId == 0, arrivalPortalControl, duration, arrivalAdmissionReleaseProgress, tmpeControlledEntrance)
                : new PortalAnimationRequest(info, surfaceOnlyDeparture ? portal : underground, arrivalApproach, portal, lane, false, departureRotation, surfaceColor, facility.Id, vehicleId, facility.TargetBuildingId == 0, arrivalPortalControl, duration, 1f);

            lock (Sync)
            {
                HashSet<int> activeDirection = arrival
                    ? ActiveArrivalFacilities
                    : ActiveDepartureFacilities;
                if (Pending.Count >= MaxQueuedAnimations
                    || activeDirection.Contains(facility.Id))
                    return false;
                activeDirection.Add(facility.Id);
        ...
        private sealed class PortalAnimationDriver : MonoBehaviour
        {
            ...
            private void Update()
            {
                ...
                for (int i = _active.Count - 1; i >= 0; i--)
                {
                    ...
                    if (animation.WaitingForArrivalHandoff)
                    {
                        ...
                        // Both parking modes publish the same visible handoff:
                        // keep sampling the exact native render pose until its
                        // simulation owner has disappeared. TM:PE retains and
                        // unspawns its native record; ordinary parking commits
                        // and retires it, but neither may start from an older
                        // request-time pose.
                        UndergroundParkingOccupancyHarmony
                    ...
                    // The native road car may still be moving at this exact
                    // pose. Arrival must continue on the first animation
                    // update; smoothstep's zero initial derivative would add
                    // an artificial stop at the handoff. Linear path time and
                    // the shared 13.4112 m/s surface/internal speed prevent a
                    // following managed car from running faster than the car
                    // ahead. Departures keep their accepted eased presentation.
                    ...
                    if (animation.Arrival
                        ...
                        && t >= animation.ArrivalAdmissionReleaseProgress)
                    {
                        ScheduleArrivalAdmissionRelease(animation);
                        if (_arrivalClearanceLogCount++ < 64)
                        {
                            UndergroundParkingLog.Advanced(
                                "UPG arrival cleared entrance for following traffic: facility="
                                + animation.FacilityId
                                + " vehicle=" + animation.VehicleId
                                + " progress="
                                + animation.ArrivalAdmissionReleaseProgress.ToString("0.000")
                                + " visualJourneyContinues=True");
                        }
                    }
            ...
            internal int DrawXrayDepartures(Camera camera)
            {
                if (camera == null)
                    return 0;
                int rendered = 0;
                for (int i = 0; i < _active.Count; i++)
                {
                    ...
                    if (animation == null
                        ...
                        || animation.Root == null
                        || animation.Info == null
                        || animation.Info.m_mesh == null
                        || animation.Info.m_material == null
                        || !UndergroundParkingVisualManager.IsFacilityVisibleOnCamera(
                            animation.FacilityId))
                        continue;

                    Material material = animation.Info.m_material;
                    Color originalColor = material.color;
                    material.color = animation.SurfaceColor;
                    int subMeshCount = Mathf.Max(1, animation.Info.m_mesh.subMeshCount);
                    for (int subMesh = 0; subMesh < subMeshCount; subMesh++)
                    {
                        int pass = Mathf.Min(subMesh, material.passCount - 1);
                        if (pass < 0 || !material.SetPass(pass))
                            continue;
                        Graphics.DrawMeshNow(
                            animation.Info.m_mesh,
                            animation.Root.transform.localToWorldMatrix,
                            subMesh);
                        rendered++;
                    }
            ...
            private static void RebaseArrivalPath(
                PortalAnimation animation,
                Vector3 exactPosition,
                Quaternion exactRotation)
            {
                Vector3 previousStart = animation.Start;
                // The rendered body's forward vector is the authoritative
                // first derivative. Reusing the request-time Bezier handle
                // after a render-pose rebase can point across the carriageway
                // and is the source of the observed snap-turn.
                Vector3 laneDirection = NormalizeFlat(
                ...
                if (UndergroundParkingOccupancyManager.TryGetPortalForFacility(
                        ...
                        out connection))
                {
                    animation.ArrivalAdmissionReleaseProgress =
                        CalculateArrivalAdmissionReleaseProgress(
                            animation.Info,
                            facility,
                            animation.Start,
                ...
                if (_handoffRebaseLogCount++ < 64)
                {
                    ...
                    UndergroundParkingLog.Advanced(
                        "UPG arrival path rebased at last rendered pose: vehicle="
                        + animation.VehicleId
                        + " facility="
                        + animation.FacilityId
                        + " delta="
                        ...
                        + " tangent=exact-rendered-forward"
                        + " controls=continuous-perpendicular-entry");
                }
            }
            ...
            private static PortalAnimation CreateAnimation(PortalAnimationRequest request)
            {
                ...
                PortalAnimation animation = new PortalAnimation(
                    ...
                    request.Duration,
                    request.ArrivalAdmissionReleaseProgress,
        ...
        private struct PortalAnimationRequest
        {
            ...
            public readonly float ArrivalAdmissionReleaseProgress;
            ...
            public PortalAnimationRequest(
                ...
                Vector3 portalControl,
                float duration,
                float arrivalAdmissionReleaseProgress,
                bool controlledTmpeEntrance = false)
            {
                ...
                Duration = duration;
                ArrivalAdmissionReleaseProgress = arrivalAdmissionReleaseProgress;
        ...
        private sealed class PortalAnimation
        {
            ...
            public float ArrivalAdmissionReleaseProgress;
            public readonly bool ControlledTmpeEntrance;
            public readonly Color SurfaceColor;
        ...
        private static void UpdateArrivalAdmissionCooldowns(float delta)
        {
            ...
            lock (Sync)
            {
                     ...
                     i++)
                {
                    ...
                    ActiveArrivalFacilities.Remove(facilityId);
                }
                ArrivalAdmissionCooldownFacilityBuffer.Clear();
            }
        }
        ...
        private static void ReleaseFacility(int facilityId, bool arrival)
        {
            ...
            lock (Sync)
            {
                ...
                HashSet<int> activeDirection = arrival
                    ? ActiveArrivalFacilities
                    : ActiveDepartureFacilities;
                activeDirection.Remove(facilityId);
            }
        }
    }
}
